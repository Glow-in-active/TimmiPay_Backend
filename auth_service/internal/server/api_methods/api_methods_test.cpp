 ...
 *
 * Эта функция используется c `curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, CurlWriteCallback)`
 * для сохранения ответа сервера в строковую переменную.
 *
...
public:
     ...
     *
     * Инициализирует внутренние зависимости с использованием тестовых конфигураций
     * PostgreSQL и Redis.
     */
    RealSessionStart()
...
public:
     ...
     *
     * Инициализирует внутренние зависимости с использованием тестовой конфигурации Redis.
     */
    RealSessionHold()
        : redis_config_(load_redis_config("database_config/test_redis_config.json")),
          redis_(connect_to_redis(redis_config_)),
...
protected:
     ...
     *
     * Регистрирует маршруты API, запускает сервер Crow в отдельном потоке и ожидает его готовности.
     */
     ...
     *
     * Выполняет HEAD-запрос к эндпоинту `/session_start` и проверяет успешность соединения.
     *
