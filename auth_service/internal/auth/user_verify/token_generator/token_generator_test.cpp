...
class TokenGeneratorTest : public ::testing::Test {
 protected:
   ...
   *
   * Загружает конфигурацию Redis, устанавливает соединение и очищает базу
   * данных Redis.
   */
   ...
   */
  void SetUp() override { redis->flushdb(); }
...
/**
 * @brief Проверяет, что TokenGenerator генерирует валидный токен и сохраняет
 * его в Redis.
 *
 * Тест проверяет формат сгенерированного токена (наличие дефисов, длину) и
 * убеждается, что токен успешно сохраняется в Redis с правильным ID
 * пользователя и TTL.
 */
