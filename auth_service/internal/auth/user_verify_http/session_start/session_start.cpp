 ...
 *
 * Извлекает email и хеш пароля из данных запроса, использует `UserVerifier` для
 * генерации токена. Возвращает токен в случае успеха или ошибку при неверном
 * формате JSON или неудачной верификации.
 *
...
json SessionStart::HandleRequest(const json& request_data) {
  try {
    ...
    const std::string password_hash =
        request_data.at("password_hash").get<std::string>();

    const std::string token =
        user_verifier_.GenerateToken(email, password_hash);

    return json{{"token", token}};

  } catch (const json::exception& e) {
    return json{{"error", "Invalid JSON format"}, {"details", e.what()}};
  } catch (const std::exception& e) {
    return json{{"error", "Verification failed"}, {"details", e.what()}};
  }
}