...
/**
 * @brief Вспомогательный класс для доступа к приватным членам SessionHold в
 * тестах.
 *
 * Позволяет получить доступ к объекту Redis внутри SessionHold для целей
 * тестирования.
 */
class SessionHoldAccessor : public SessionHold {
 public:
   ...
   */
  sw::redis::Redis& get_redis() { return this->redis_; }
};
 ...
 * возвращать предопределенные ответы, а также управлять состоянием Redis для
 * теста.
 */
class MockSessionHold : public SessionHoldAccessor {
 public:
   ...
   *
   * Сохраняет данные входящего запроса, устанавливает токен в Redis (если он
   * присутствует) и возвращает предварительно установленный ответ.
  ...
  nlohmann::json HandleRequest(const nlohmann::json& request_data) {
    std::cout << "Request data in HandleRequest: " << request_data.dump()
              << std::endl;
