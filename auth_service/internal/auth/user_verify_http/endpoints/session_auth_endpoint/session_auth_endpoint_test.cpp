 ...
 *
 * Позволяет имитировать поведение SessionStart для тестирования обработчиков эндпоинтов,
 * фиксируя входящие запросы и возвращая предопределенные ответы.
 */
 ...
 *
 * Использует MockSessionStart для изоляции и тестирования логики эндпоинта аутентификации.
 */
...
/**
 * @brief Проверяет обработку валидного запроса к эндпоинту аутентификации сессии.
 *
 ...
 *
 * Тест имитирует ошибку формата JSON от обработчика `SessionStart` (например, отсутствующее поле)
 * и проверяет, что эндпоинт возвращает HTTP-статус 400.
 */
 ...
 *
 * Тест имитирует ошибку верификации от обработчика `SessionStart` (например, неверные учетные данные)
 * и проверяет, что эндпоинт возвращает HTTP-статус 401.
 */
 ...
 *
 * Тест имитирует общую неизвестную ошибку от обработчика `SessionStart` и проверяет,
 * что эндпоинт возвращает HTTP-статус 401 и соответствующее сообщение об ошибке.
 */
