 ...
 *
 * Настраивает и очищает тестовую среду с использованием реальных подключений к
 * PostgreSQL и Redis, а также тестовых пользователей.
 */
class ProdSessionHoldTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    txn.exec_params(
        "INSERT INTO users (email, password_hash, username) VALUES ($1, $2, "
        "$3) "
   ...
   *
   * Удаляет тестового пользователя из PostgreSQL и все связанные ключи сессий
   * из Redis.
   */
...
TEST_F(ProdSessionHoldTest, SessionHoldValidToken) {
  ...
  nlohmann::json session_request = {{"email", test_email},
                                    {"password_hash", test_hash}};
  nlohmann::json session_response =
      session_handler.HandleRequest(session_request);
...
/**
 * @brief Проверяет обработку невалидного или несуществующего токена при
 * удержании сессии.
 *
...
TEST_F(ProdSessionHoldTest, RealDbIntegration) {
  ...
  nlohmann::json request = {{"email", test_email},
                            {"password_hash", test_hash}};
