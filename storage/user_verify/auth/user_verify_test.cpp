...
/**
 * @brief Тестовый класс для UserStorage, использующий реальное соединение с
 * базой данных.
 *
...
class UserStorageProdTest : public ::testing::Test {
 protected:
   ...
   *
   * Генерирует уникальный ID пользователя и email, устанавливает соединение с
   * тестовой базой данных и вставляет тестового пользователя.
   */
  void SetUp() override {
    ...
    setup_work.exec_params(
        ...
        "VALUES ($1, $2, $3, $4)",
        test_user_id, "test_user", test_email, "test_hash");
    setup_work.commit();
  }
  ...
  void TearDown() override {
    ...
    teardown_work.exec_params("DELETE FROM users WHERE id = $1", test_user_id);
 ...
 *
 * Тест получает пользователя по email и проверяет, что его ID и email
 ...
 *
 * Тест получает пользователя по email и проверяет правильность верификации
 * пароля как для верного, так и для неверного хеша.
 */
