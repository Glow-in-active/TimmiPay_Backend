...
class RedisSetTokenTest : public ::testing::Test {
 protected:
   ...
   */
  void TearDown() override { redis->flushdb(); }
 ...
 *
 * Тест устанавливает токен в Redis, затем проверяет, что ID пользователя и
 * время истечения срока действия сохранены правильно, а TTL находится в
 * ожидаемом диапазоне.
 */
 ...
 *
 * Тест устанавливает токен и проверяет, что рассчитанное время истечения срока
 * действия находится в пределах ожидаемого диапазона относительно времени
 * установки.
 */
TEST_F(RedisSetTokenTest, ExpirationTimeIsCorrect) {
  ...
  const auto min_expected =
      start + std::chrono::minutes(10) - std::chrono::seconds(5);
  const auto max_expected =
      end + std::chrono::minutes(10) + std::chrono::seconds(5);
 ...
 *
 * Тест устанавливает токен с одним ID, затем перезаписывает его другим ID и
 * проверяет, что ID обновлен и TTL также обновлен.
 */
 ...
 *
 * Тест устанавливает токен с коротким TTL и проверяет, что токен существует до
 * истечения срока действия и удаляется после него.
 */
