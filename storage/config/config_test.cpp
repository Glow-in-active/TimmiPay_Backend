 ...
 *
 * Тест создает временный JSON-файл с валидными данными конфигурации, загружает
 * его с помощью функции `load_config` и проверяет, что все поля структуры
 * `Config` заполнены правильно.
 */
 ...
 *
 * Тест вызывает `load_config` с именем несуществующего файла и ожидает
 * исключения `std::runtime_error`.
 */
TEST(ConfigTest, ThrowsOnMissingFile) {
  EXPECT_THROW(
      { load_config("non_existent_config.json"); }, std::runtime_error);
}
 ...
 * конфигурации, вызывает `load_config` и ожидает исключения
...
TEST(ConfigTest, ThrowsOnMissingField) {
  ...
  EXPECT_THROW({ load_config(filename); }, nlohmann::json::exception);
 ...
 * вместо числа), вызывает `load_config` и ожидает исключения
 * `nlohmann::json::exception`.
 */
 ...
 * JSON.
 *
 * Тест создает файл с некорректным JSON-форматом, вызывает `load_config` и
 * ожидает исключения `nlohmann::json::parse_error`.
 */
TEST(ConfigTest, ThrowsOnMalformedJSON) {
  ...
  EXPECT_THROW({ load_config(filename); }, nlohmann::json::parse_error);
 ...
 *
 * Тест создает пустой файл, вызывает `load_config` и ожидает исключения
