...
/**
 * @brief Интеграционный тестовый класс для проверки подключения к базе данных
 * PostgreSQL.
 *
 ...
 *
 * Тест пытается установить соединение с базой данных, используя валидные
 * параметры конфигурации, и проверяет, что соединение открыто и не выбрасывает
 * исключений.
 */
...
TEST_F(DatabaseConnectionIntegrationTest, ThrowsOnInvalidHost) {
  ...
  EXPECT_THROW(
      { auto conn = connect_to_database(config); }, std::runtime_error);
}
 ...
 *
 * Тест пытается подключиться к базе данных с неверными именем пользователя
 * и/или паролем и ожидает исключения `std::runtime_error`.
 */
...
/**
 * @brief Проверяет, что выбрасывается исключение при неверном имени базы
 * данных.
 *
