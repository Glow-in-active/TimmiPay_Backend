 ...
 *
 * Настраивает и очищает тестовую базу данных и сервер Crow для каждого
 * тестового набора.
 */
class ServerTest : public ::testing::Test {
 protected:
   ...
   *
   * Инициализирует статические соединения с PostgreSQL и Redis, создает и
   * запускает FinanceServer в отдельном потоке, а затем ожидает его готовности.
   */
  static void SetUpTestSuite() {
    ...
    postgres_conn = std::make_unique<pqxx::connection>(
    ...
    server_thread = std::thread([]() { server->run(8080); });
   ...
   *
   * Останавливает FinanceServer и ожидает завершения его потока. Соединения с
   * базами данных будут закрыты автоматически при уничтожении
   * `std::unique_ptr`.
   */
  ...
  void setupTestData() {
    ...
    txn.exec_params(
        ...
        "'test_user1', 'test1@example.com', 'password_hash_1')",
        test_user1_id);
    txn.exec_params(
        "INSERT INTO users (id, username, email, password_hash) VALUES ($1, "
        "'test_user2', 'test2@example.com', 'password_hash_2')",
        test_user2_id);

    test_usd_id = uuid_gen.generateUUID();
    test_eur_id = uuid_gen.generateUUID();
    txn.exec_params(
        "INSERT INTO currencies (id, code, name) VALUES ($1, 'USD', 'United "
        "States Dollar')",
        test_usd_id);
    txn.exec_params(
        "INSERT INTO currencies (id, code, name) VALUES ($1, 'EUR', 'Euro')",
        test_eur_id);

    txn.exec_params(
        "INSERT INTO accounts (id, user_id, currency_id, balance) VALUES ($1, "
        "$2, $3, 1000.0)",
        uuid_gen.generateUUID(), test_user1_id, test_usd_id);
    txn.exec_params(
        "INSERT INTO accounts (id, user_id, currency_id, balance) VALUES ($1, "
        "$2, $3, 500.0)",
        uuid_gen.generateUUID(), test_user2_id, test_usd_id);
   ...
   *
   * Удаляет тестовых пользователей, счета, валюты из PostgreSQL и тестовый
   * токен сессии из Redis.
   */
  ...
  std::string makeRequest(const std::string& endpoint,
    ...
    if (curl) {
      curl_easy_setopt(curl, CURLOPT_URL,
                       ("http://localhost:8080" + endpoint).c_str());
...
TEST_F(ServerTest, GetBalanceSuccess) {
  nlohmann::json request_data = {{"session_token", test_session_token}};

  std::string response =
      makeRequest("/api/v1/balance", "POST", request_data.dump());
...
/**
 * @brief Проверяет обработку недействительного токена сессии при получении
 * баланса.
 *
...
TEST_F(ServerTest, GetBalanceInvalidSession) {
  nlohmann::json request_data = {{"session_token", "invalid_token"}};
...
TEST_F(ServerTest, TransferMoneySuccess) {
  ...
  std::string response =
      makeRequest("/api/v1/transfer", "POST", request_data.dump());
  auto response_json = nlohmann::json::parse(response);
 ...
 *
 * Тест отправляет запрос на перевод суммы, превышающей доступный баланс
 * пользователя, и проверяет, что сервер возвращает соответствующее сообщение об
 * ошибке.
 */
 ...
 *
 * Тест сначала выполняет перевод, а затем отправляет запрос на получение
 * истории транзакций. Проверяет, что ответ содержит корректные данные о
 * транзакции, включая сумму и статус.
 */
TEST_F(ServerTest, GetTransactionHistory) {
  // Make a transfer first
  nlohmann::json transfer_data = {{"session_token", test_session_token},
                                  {"to_username", "test_user2"},
                                  {"amount", 100.0},
                                  {"currency", "USD"}};
  ...
  nlohmann::json history_data = {
      {"session_token", test_session_token}, {"page", 1}, {"limit", 10}};

  std::string response =
      makeRequest("/api/v1/history", "POST", history_data.dump());
  auto response_json = nlohmann::json::parse(response);
